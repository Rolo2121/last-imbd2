            ...
            User.findOneAndUpdate({ _id: req.user._id },
                {
                    $set: {
                        ...
                        lastName: santizie(req.body.lastName),
                        image: imgURL
                    }
                }, (err, doc) => {
                    if (err) return res.json({ success: false, err });
    ...
    try {
        User.findOne({ tokenConf: token })
            .exec(function (err, user) {
                ...
                else if (!user) throw new Error('error user not found')
                else {
    ...
    User.findOne({ username: sanitize(req.body.username) }, (err, user) => {
        ...
        user.comparePassword(sanitize(req.body.password), (err, isMatch) => {
            ...
            user.CheckTokenMail(sanitize(req.body.username), (err, tokenfind) => {
                ...
                if (!tokenfind)
                    return res.json({ loginSuccess: false, message: 'check your email' });
...
router.post('/forgotPassword', async (req, res) => {
    try {
        ...
        if (user) {
            ...
            const mailOptions = {
                from: {
                    name: "The-Last-IMDB Team",
                    email: "email"},
                  to: sanitize(user.email),
                subject: 'The-Last-IMDB | Password Reset',
                html: html,
            }
